...
def is_correct_squat(hip_angle, knee_angle):
    return (70 <= knee_angle <= 190 and hip_angle < 100)

def is_wrong_position(hip_angle, knee_angle):
    return ((70 > knee_angle or knee_angle > 110) or (hip_angle > 110 or hip_angle <70))
        ...
        if results.pose_landmarks:
            ...
            if ready:
                if hip_angle < 90:
                    if stage != 'down':
                      stage = 'down'
                    if is_wrong_position(hip_angle, knee_angle):
                      feedback = "Incorrect posture! Adjust your form."
                if hip_angle > 160:
                    if stage == 'down':
